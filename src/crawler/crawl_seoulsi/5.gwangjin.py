...
# 🗺️ 광진구 대략 범위 (bounding box)
# (북쪽: 중곡·군자동 / 남쪽: 한강(자양동, 구의동) / 서쪽: 건대입구·화양동 / 동쪽: 중랑천)
xmin, xmax = 127.07, 127.12   # 경도 (longitude)
ymin, ymax = 37.53, 37.57     # 위도 (latitude)
...
for idx, (x, y) in enumerate(coords, start=1):
    ...
    for c in cafes:
        ...
        if not address or "광진구" not in address:  # 광진구만 저장
            continue
